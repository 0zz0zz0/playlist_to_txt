                ...
                match get_url(&mut input) {
                    Ok(url) => match get_idstr(url) {
                        Ok(idstr) => match get_id(idstr) {
                            ...
                            Err(_) => {clear(); println!("Input was a URL but did not contain a valid spotify id");}
                        },
                        Err(_) => {clear(); println!("Input was a URL but did not contain a valid spotify id")}
                    },
                    Err(_) => {clear(); println!("Input was not a valid URL")}
                }
            }
        }
    }
...
async fn main() {
    ...
    println!("Writing to file, please wait...");
